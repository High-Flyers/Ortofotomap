...
class CameraObject:
    ...
    @app.route('/repeat_streaming_<int:camera_num>', methods=['POST'])
    def repeat_streaming(camera_num):
        try:
            # Pobierz obiekt kamery na podstawie numeru kamery
            camera = cameras.get(camera_num)
            if not camera:
                return jsonify(success=False, message=f"Camera {camera_num} not found.")
            
            # Wznów streaming
            camera.start_streaming()
            return jsonify(success=True, message=f"Streaming resumed for camera {camera_num}.")
        except Exception as e:
            # Obsługa błędów i logowanie
            logging.error(f"Error resuming streaming for camera {camera_num}: {e}")
...
def start_recording(camera_num):
    try:
        ...
        # Określ ścieżkę zapisu nagrania
        #output_path = os.path.join(app.config['UPLOAD_FOLDER'], f"recording_camera_{camera_num}.mjpeg")
        
        # Generowanie unikalnej nazwy pliku z timestampem
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(app.config['UPLOAD_FOLDER'], f"recording_camera_{camera_num}_{timestamp}.mjpeg")
